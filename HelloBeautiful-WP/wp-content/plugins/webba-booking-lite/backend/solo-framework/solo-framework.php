...
function slfLoadAssets(){
	if ( isset( $_GET[ 'page' ] ) && ( $_GET[ 'page' ] == 'wbk-appearance'   ||   $_GET[ 'page' ] == 'wbk-appointments' || $_GET[ 'page' ] == 'wbk-email-templates' || $_GET[ 'page' ] == 'wbk-service-categories'  || $_GET[ 'page' ] == 'wbk-gg-calendars'  ) ) { 
...
class SoloFramework extends stdClass  {
	...
	public function renderSectionSetControls( $slug ){
		...
		$html .= '<input  value="'. __( 'Load presets', 'slf' ) . '" type="button" class="button slf-deserialize-button slf_right" onclick = "slf_load_presets( \'' . $this->slug . '\',  \'' . $slug . '\')">';
 		$html .= $select;
	...
	public function adminFunctions( $slug ){
		...
		$html .= '<input  value="'. __( 'Serialize options', 'slf' ) . '" type="button" class="button slf-serialize-button" onclick = "slf_serialize_sections_set( \'' . $this->slug . '\',  \'' . $slug . '\')">';
		$html .= '</p>';
		return $html;
	}
}
