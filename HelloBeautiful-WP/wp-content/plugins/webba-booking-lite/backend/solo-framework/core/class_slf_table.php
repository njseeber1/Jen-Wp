...
function getAllowedClassNames() {
	return array( 'WBK_Appointments_Table' , 'WBK_Email_Templates_Table', 'WBK_Service_Categories_Table', 'WBK_Service_Categories_Table', 'WBK_GG_Calendar_Table' );
}
...
class SLFTable extends stdClass {
	...
	public function renderAddRowForm(){
		$html = '<h3>' . __( 'Fill in a form to add new element:') . '</h3>';
		foreach ( $this->field_set->fields as $field ) { 
			if ( !$field->render_control ){
				continue;
			}		 
