...
function wbk_renderSetDate( scroll ) {
	...
	if( wbkl10n.date_input == 'popup' ){	
		if ( wbk_service_limits[selected_service_id] != '' ){
			...
			if( wbk_is_int( wbk_service_limits[selected_service_id] ) ){
				jQuery('#wbk-date-container').html('<input value="' +  wbk_service_limits[selected_service_id] + '" type="hidden" name="wbk-date_submit" class="wbk-input" id="wbk-date" />');
				wbk_clearForm();
         		wbk_clearDone();
         		wbk_clearTimeslots();
         		wbk_clearSetTime();
	...
	if( wbkl10n.date_input == 'popup' ){	
		...
		if( range_min != undefined ){
		 ...
		 if ( wbk_available_days[selected_service_id] != '' ){
		 	var options_html = '<option value="0">' + wbkl10n.selectdate + '</option>';
			var day_abilities = wbk_available_days[selected_service_id].split(';');			 
