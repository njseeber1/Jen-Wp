...
class WBK_Service_Schedule {
	...
	public function buildSchedule( $day, $ignore_optimization = false ){
		$this->breakers = array();
		// load appointments
				...
				if ( $timeslot->getStart() >= $time_after ) {
					if ( $timeslot->getStart() > time() ) {
						...
						if ( $timeslot_format == 'detailed' ){
							...
							$book_button = '<input type="button" data-end="' . $timeslot->getEnd() . '"  data-start="' . $timeslot->getStart() . '"  value="' . $book_text .'" id="wbk-timeslot-btn_' . $timeslot->getStart() . '" data-available="' . $available_count . '"   class="wbk-slot-button" />';
							} else {
								$slot_button =  get_option ( 'wbk_booked_text', '' );
								if( $slot_button == '' ){
									$slot_button = sanitize_text_field( $wbk_wording['booked_text'] );
								}		 
								$book_button = '<input type="button" value="' . $slot_button .'" class="wbk-slot-button wbk-slot-booked" />';
							}
							$slot_html ='<div class="wbk-slot-time">' . $time . '<br>' .  $local_time_str . '</div>'. $available_html . $book_button;
						} else {
							if ( $this->service->getQuantity() > 1 ){
								$available_count = $this->getAvailableCount( $timeslot->getStart() );
								if ( $available_count == 0 ) {
							...
							$slot_html = '<input type="button"  data-end="' . $timeslot->getEnd() . '"  data-start="' . $timeslot->getStart() . '" value="' . $time .'" id="wbk-timeslot-btn_' . $timeslot->getStart() . '" data-available="' . $available_count . '"   class="wbk-slot-button" />';
						}
				...
				if( $timeslot->getStatus() == -2 ){
					if( get_option( 'wbk_show_booked_slots', 'disabled' ) == 'enabled'){
						...
						if ( $timeslot_format == 'detailed' ){
							...
							$slot_html = '<input type="button" value="' . $slot_button .'" class="wbk-slot-button wbk-slot-booked" />';
