...
class WBK_Ajax_Controller {
	...
	//**TZ 	date_default_timezone_set( 'UTC' );
		...
		for ( $i = 1;  $i <= 7;  $i++ ) {
 		...
 		//**TZ $timezone = get_option( 'wbk_timezone', '' );
        //**TZ if ( $timezone != '' ){
	...
	public function ajaxRenderBookingForm() {
	    ...
	    $result = $this->render_booking_form( $service_id, $time );
	    if( $result === FALSE ){
	    	echo -1;
	    	date_default_timezone_set('UTC');
	    	die();
	...
	public function ajaxBook() {
		...
		if( get_option( 'wbk_multi_booking', 'disabled' ) != 'disabled' &&  get_option( 'wbk_email_customer_book_multiple_mode', 'one' ) ) {
			$noifications = new WBK_Email_Notifications( $service_id, $appointment_id, $current_category );
